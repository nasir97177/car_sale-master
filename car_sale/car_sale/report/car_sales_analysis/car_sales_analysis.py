...
CASE WHEN COALESCE(SIT.QTY,0)> 0 THEN SN.purchase_rate WHEN COALESCE(SIT.QTY,0)< 0 THEN (-1 * SN.purchase_rate) ELSE 0 END  as PurchaseRate,

SN.plate_cost AS PlateCost,

CASE WHEN COALESCE(SIT.QTY,0) > 0 THEN SIT.base_net_rate WHEN COALESCE(SIT.QTY,0)<0 THEN (-1 * SIT.base_net_rate) ELSE 0 END as TotalSelling,
